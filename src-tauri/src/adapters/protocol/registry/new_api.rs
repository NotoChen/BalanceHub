...
pub(super) const DEFINITION: ProviderProtocolDefinition = ProviderProtocolDefinition {
    ...
    auth_schemas: &[
        ProviderProtocolAuthSchema::new(
            ...
            "账号密码",
            "登录并建立会话",
            "保存后首次同步会登录站点并缓存会话；开启 2FA、验证码或第三方登录时，可使用“登录并导入”。",
        ...
        ProviderProtocolAuthSchema::new(
            ...
            "登录会话",
            "使用浏览器登录或已有 Cookie",
            "通过“登录并导入”建立会话，也可手动填写已有 Cookie。",
            &[],
            &["apiUser", "accessToken", "apiKey"],
            SESSION_FIELDS,
        )
        .with_required_any_fields(&["sessionCookie", "newApiSession.refreshCookie"]),
        ProviderProtocolAuthSchema::new(
    ...
    credential_assistant: ProviderCredentialAssistantDefinition {
        ...
        access_token_skip_fields: &["newApiSession.refreshCookie"],
        api_key_required_fields: &["apiUser"],
        api_key_required_any_fields: &["sessionCookie", "accessToken", "newApiSession.refreshCookie"],
    },
    credentials: &ADAPTER,
    connection: &ADAPTER,
