...
impl<'a> ProviderService<'a> {
    pub async fn probe_site(
        ...
        NewApiAdapter.probe_site(&data.settings, &provider).await
    }
    ...
    pub async fn probe_capabilities(
        ...
        let (capabilities, invite_link, error) = NewApiAdapter
        ...
        let (providers, updated_provider, message) = self.mutate(|data| {
            ...
            let mut message = "站点能力已探测".to_string();
                ...
                if let Some(result) = models_result {
                    match result {
                        Ok(models) => {
                            ...
                            if !models.is_empty() {
                                message = format!("站点能力已探测，已获取 {} 个模型", models.len());
                            }
                        }
                        Err(err) => {
                            message = format!("站点能力已探测，模型列表获取失败：{err}");
                        }
                    }
                }
    ...
    pub async fn invite_link(&self, id: String) -> Result<String, String> {
        ...
        let invite_link = NewApiAdapter.invite_link(&data.settings, &provider).await?;
        let stored_link = invite_link.clone();
        self.mutate(|data| {
            if let Some(stored_provider) = data
                .providers
