...
Flow: `trellis-implement` -> required mechanical validation -> risk-triggered `trellis-check` -> `trellis-update-spec` -> commit (Phase 3.4) -> `/trellis:finish-work`.
Main-session default: dispatch implementation and independent work units. Dispatch `trellis-check` only for behavioral, architectural, cross-layer, concurrency, security, performance, compatibility, or regression-risk review; pure lint/test/build/CI collection stays with the execution agent. Sub-agent self-exemption: if already running as `trellis-implement`, do NOT spawn another `trellis-implement` or `trellis-check`; if already running as `trellis-check`, do NOT spawn another `trellis-check` or `trellis-implement`. Dispatch is main session only.
...
Flow: `trellis-before-dev` -> edit -> required mechanical validation -> risk-triggered `trellis-check` -> `trellis-update-spec` -> commit (Phase 3.4) -> `/trellis:finish-work`.
...
- **Decision boundary**: If implementation exposes an unplanned design choice, ambiguous ownership, or an unproven root cause, stop that work unit and return the evidence to the main session for `gpt-5.6-sol/max` analysis. Do not guess through it.
...
- **Decision boundary**: If implementation exposes an unplanned design choice, ambiguous ownership, or an unproven root cause, stop that work unit and return the evidence to the main session for deep analysis. Do not guess through it.
...
Mechanical validation is always required. Run the exact lint, type-check, test, build, or packaging commands defined by the task and affected package specs, and record pass/fail evidence. For Codex, command-only verification and known mechanical fixes use a `gpt-5.6-luna/max` execution agent; they do not require `trellis-check`.

Deep review is risk-triggered. Spawn or load `trellis-check` when at least one of these is true:

- behavior, architecture, public contracts, or module ownership changed;
- frontend, IPC, Rust, storage, protocol, or platform layers interact;
- concurrency, security, performance, compatibility, or regression risk needs judgment;
- the root cause or correctness of the abstraction is not already proven;
- the user explicitly requests a review.

When no trigger applies, record `trellis-check: not triggered` and the reason in task progress or the final verification record. Do not spend a deep-review agent merely to repeat deterministic commands.

[Claude Code, Cursor, OpenCode, codex-sub-agent, Kiro, Gemini, Qoder, CodeBuddy, Copilot, Droid, Pi, Oh My Pi, ZCode, Snow, Reasonix, Trae, Grok, Kimi Code]

When a deep-review trigger applies, spawn the check sub-agent:
...
When a deep-review trigger applies, load the `trellis-check` skill and verify the code per its guidance:
- Spec compliance
...
**Final pass (before Phase 3.4 commit)**: the last mechanical validation of a task must run full-scope, not just on the latest implement chunk. List all affected packages with `python3 ./.trellis/scripts/get_context.py --mode packages`, then load each package's spec index Quality Check section. If the aggregate diff meets a deep-review trigger, the last `trellis-check` must also cover the full scope. This catches cross-layer / multi-package issues a mid-iteration local 2.2 cannot.
