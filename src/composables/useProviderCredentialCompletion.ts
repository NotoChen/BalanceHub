...
export function useProviderCredentialCompletion(options: UseProviderCredentialCompletionOptions) {
  ...
  async function completeCredentials(runOptions: CompletionRunOptions = {}) {
    ...
    try {
      ...
      options.setApiKeyOptions(result.apiKeyOptions);
      options.credentialCompletionSteps.value = result.steps;
      if (result.changedFields.length > 0) {
        options.credentialCompletionMessage.value = `已补全：${result.changedFields.map(fieldLabel).join("、")}`;
        if (save) {
  ...
  async function ensureAccessToken() {
    if (options.draftProvider.auth.mode !== "session" || options.draftProvider.auth.accessToken.trim()) {
      if (options.draftProvider.auth.mode === "session") {
    ...
    if (!confirmed) {
      failAssistantStep("accessToken", "已取消生成访问令牌，配置未保存");
      return false;
    }
  ...
  async function ensureApiKey() {
    if (options.draftProvider.auth.mode === "apiKey" || options.draftProvider.auth.apiKey.trim()) {
      if (options.draftProvider.auth.mode !== "apiKey") {
        setAssistantStep("apiKey", "创建 API 密钥", "skipped", "已存在 API 密钥");
      }
      return true;
    }
    ...
    if (!name) {
      failAssistantStep("apiKey", "已取消创建 API 密钥，配置未保存");
      return false;
    }
    ...
    try {
      ...
      options.setApiKeyOptions([option]);
  ...
  function unresolvedCompletionFailures() {
    return options.credentialCompletionSteps.value.filter((step) => {
      ...
      if (step.name.includes("访问令牌") && options.draftProvider.auth.accessToken.trim()) {
        return false;
      }
      if (step.name.includes("API 密钥") && options.draftProvider.auth.apiKey.trim()) {
        return false;
      }
      return true;
    });
