...
interface UseProviderCardToneOptions {
  ...
  checkingInProviderId: Ref<string | null>;
  syncingCapabilitiesProviderId: Ref<string | null>;
...
export function useProviderCardTone(options: UseProviderCardToneOptions) {
  function providerIntermediateLabel(provider: Provider) {
    if (options.syncingCapabilitiesProviderId.value === provider.identity.id) {
      return "探测中";
    }
    if (options.checkingInProviderId.value === provider.identity.id) {
      return "签到中";
    }
