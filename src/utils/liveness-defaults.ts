...
export const MIN_CODEX_LIVENESS_INTERVAL = 45 * 60;
export const DEFAULT_LIVENESS_INTERVAL = MIN_CODEX_LIVENESS_INTERVAL;
export const DEFAULT_LIVENESS_RANDOM_MIN_INTERVAL = MIN_CODEX_LIVENESS_INTERVAL;
...
export function normalizeLivenessTiming<T extends LivenessTiming | ProviderLivenessTiming>(value: T): T {
  if ("interval" in value) {
    value.interval = Math.max(MIN_CODEX_LIVENESS_INTERVAL, Number(value.interval) || 0);
    value.randomMinInterval = Math.max(
      MIN_CODEX_LIVENESS_INTERVAL,
  ...
  value.livenessInterval = Math.max(MIN_CODEX_LIVENESS_INTERVAL, Number(value.livenessInterval) || 0);
