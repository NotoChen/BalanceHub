...
function remove() {
  Modal.confirm({ title: "卸载浏览器签到组件", content: "将删除组件和本地验证会话，保留中转站配置与签到记录。",
...
<template>
  <a-modal v-if="runtime" v-model:visible="runtime.visible.value" :width="570" :footer="false" modal-class="surface-modal">
    <template #title><span class="browser-component-heading"><Globe :size="20" /> 浏览器辅助签到</span></template>
    <div class="browser-component-content">
      <p>遇到站点验证时，使用独立的浏览器会话继续签到。组件按需安装，普通签到无需安装。</p>
      ...
      <p class="browser-component-note">每次浏览器签到前检查环境。组件版本随 App 管理，更新需确认；等待人工验证的任务可在卡片或后台任务中继续。</p>
