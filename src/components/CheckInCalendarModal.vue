...
const calendarDays = computed(() => {
  ...
  const cells: { key: string; date: string; day: number | null; record: ProviderCheckInRecord | null }[] = [];

  for (let index = 0; index < startOffset; index += 1) {
    cells.push({ key: `empty-start-${index}`, date: "", day: null, record: null });
  }
  ...
  while (cells.length % 7 !== 0) {
    cells.push({ key: `empty-end-${cells.length}`, date: "", day: null, record: null });
  }
  return cells;
});
    ...
    <div class="checkin-calendar-panel">
      ...
      <a-spin :loading="loading">
        <div class="checkin-calendar-summary">
          ...
          <p>{{ result?.message || "按月份从中转站接口增量拉取签到记录。" }}</p>
        </div>
