...
const statCards = computed(() => {
  const stats = props.result?.stats ?? { quota: 0, rpm: 0, tpm: 0 };
  return [
    {
      label: "消耗",
      value: formatLogQuotaValue(stats.quota, props.result?.quotaDisplay),
      tone: "cost",
    },
    {
      label: "RPM",
      value: formatNumberCompact(stats.rpm || 0, 2),
      tone: "rpm",
    },
    {
      label: "TPM",
      value: formatNumberCompact(stats.tpm || 0, 2),
      tone: "tpm",
    },
    {
      label: "记录",
      value: formatNumberCompact(props.result?.total ?? rows.value.length, 0),
      tone: "count",
    },
  ];
});
