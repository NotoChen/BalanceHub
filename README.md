...
- **衔接本地工具**：右键中转站可临时启动 Codex / Claude Code CLI，也可以把当前配置添加到 CC Switch。
- **本地优先**：Cookie、Token、API Key 和中转站配置保存在本机，后端请求由 Tauri / Rust 在本地执行。
...
<p>
  <img src="docs/assets/screenshots/context-menu-expanded.png" width="49%" alt="右键菜单" />
...
3. 选择认证方式，优先使用 Cookie，其次是访问令牌或 API Key。
4. 测试连接并保存，中转站会出现在主面板。
5. 按需开启自动刷新、自动签到、自动测活和通知。
6. 需要临时使用某个中转站时，在卡片右键菜单中选择 Codex / Claude Code，选择工作目录后启动终端。
...
| 测活 | Codex / Claude Code CLI 测活、HTTP 测活、候选 CLI 扫描、时间线记录。 | 判断站点、模型、Key、代理或本机 CLI 是否可用。 |
...
默认优先级是 Cookie > 访问令牌 > API Key。账号额度、签到、API Key 管理等能力通常需要 Cookie 或访问令牌；只关心 Key 维度额度时可以使用 API Key。
...
会。CLI 测活和 HTTP 测活都会向中转站发起真实请求。手动测活立即执行；自动测活首次开启前会要求确认。
